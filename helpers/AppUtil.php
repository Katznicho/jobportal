...
include 'Loans.php';
require_once('schedulepenalty.php');
require_once('aftermaturitypenalty.php');
class AppUtil
{
    ...
    public static function loanSummaryTable($loans, $db, $borrower)
    {
?>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <?php
                            $repay_dates = Loans::repaymentDates($loan);
                        ...
                        <td>
                            <?php
                            $installments = Loans::totalAmountsToPay($loan, $paidInstallments);
                        ...
                        <td>
                            <?php
                            if (!is_null($loan['overriden_due'])) {
                                $totalDue = (int)$loan['overriden_due'];
                            }
                            if ((int)($totalDue + $afterMP + $sumPenalty - $totalPaid) <= 0) {
                            ?>
                                <span class="label label-success">Fully Paid</span>
                            <?php
                            } else  if ($status['missed_repayment'] == 1 && $status['past_maturity'] == 0) {
                            ?>
                                <span class="label label-info">Missed Repayment</span>
                            <?php
                            } else if ($status['missed_repayment'] == 1 && $status['past_maturity'] == 1) {
                            ?>
    ...
    public static function nextDueDate($db, $loanDetails)
    {
        ...
        $loan_totals = Loans::totalAmountsToPay($loanDetails, $repayments);
        ...
        if ($total_pending_amount > 0) {
            ...
            for ($i = 1; $i <= count($Repayment_dates); $i++) {
                $installment = Loans::getInstallmentAmount($loanDetails, $Remaining_principle, $i)[2];
        ...
        if (!$collection_date && !is_null($loanDetails["overriden_due"]) && $loanDetails["overriden_due"] > $total_amount_paid) {
            // $Remaining_principle = $loanDetails["overriden_due"];
            return $Repayment_dates[count($Repayment_dates) - 1];
        }
    ...
    public static function Loan_status($db, $loanDetails)
    {
        ...
        $loan_totals = Loans::totalAmountsToPay($loanDetails, $allRepayments);

        $num_of_installments = Loans::numberOfInstallments($loanDetails);
    ...
    public static function getCcEmails($companyId)
    {
        return array(
            ...
            [
                ...
                "email" => "knicholas@thinkxsoftware.com"
            ],
            [
                "name" => "Wandera Timothy",
                "email" => "wtimothy@thinkxsoftware.com"
            ]
...
<?php
    ...
    public static  function getReminders()
    {
        ...
        return  $manager_db->select('reminders', [], ['company_id' => self::companyId(), 'is_deleted' => 0]);
    }



    // Method to be called during updating or any change on the


    public static function updateLoanOptimizedView($db, $loan_id)
    {

        $loans = $db->select("loans", [], ['id' => $loan_id]);
        ...
        foreach ($loans as $loan) {
            ...
            if (is_null($loan['overriden_matutity_date'])) {
                $maturity_date = AppUtil::Comparable_date_format($dates[count($dates) - 1]);
            } else {
                $maturity_date = $loan['overriden_matutity_date']; #AppUtil::Comparable_date_format($dates[count($dates) - 1]);

            }

            $borrowerDetails = "";
            if ($loan['is_group'] == 1) {
                $borrowerDetails = $db->select("borrowers_group", ['Id', 'name'], ['id' => $loan['borrower_id']])[0];
            } else {
                $borrowerDetails = $db->select("borrower", ['id', 'fname', 'lname', 'title'], ['id' => $loan['borrower_id']])[0];
            }
            ...
            $balance = 0;
            if (is_null($loan['overriden_due'])) {
                $balance = ($Installments[3] + $afterMP + $sumPenalty) - $Installments[2];

            } else {
                $balance = ($loan['overriden_due'] + $afterMP + $sumPenalty) - $Installments[2];
            }
            if (count($payments) > 0) {
                $lastRepayment = $payments[(count($payments) - 1)];
                $lp = $lastRepayment['collection_date'];
            } else {
                $lp = "no payments";
            }
            ...
            if ((int)$balance <= 0) {
                ...
                if ($status['missed_repayment'] == 1) {
                    // Get the missed day
                    $s = 'Missed Repayment';
                    $missedDay = $status['missed_day'];
                    $daysPastRepaymentDate = AppUtil::getDateDiff($missedDay, date("Y-m-d"));
            ...
            $data = [
                ...
                'paid' => $paid,
                'status' => $s,
                'due' => is_null($loan['overriden_due']) ? $totalDue : $loan['overriden_due'],
                'collector' => $loan['collector'],
                'missed_days' => $s == 'Missed Repayment' ? $daysPastRepaymentDate : 0,
                'last_payment_date' => $lp
            ];

            $res = $db->select('loan_status_table', [], ['loan_id' => $loan['id']]);
            if (count($res) == 0) {
                // new Loan Entry
                $id = $db->insert('loan_status_table', $data);
                //    var_dump($id);
            } else {
                //    requires updating
                // echo 'updating - '. $loan['id'] . '<br/>';
                $db->update('loan_status_table', $data, ['loan_id' => $loan['id']]);
            }
        }
    }
}
